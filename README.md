...
一个单文件即可使用的素材收集导航页,收录 **23 大类、270+ 免费可商用站点**:

图片 · 壁纸 / 背景 · 视频 · BGM · 游戏 · 贴图纹理 · 动态素材 · 游戏角色 · UI 参考 · 图标 · 字体 · 设计工具 · 电子书 · 教育备课 · Skill 技能 · MCP 合集 · 提示词库 · AI 工具站 · 语音配音 · 白噪音 · 音乐直链 · 像素素材 · 海洋专题
...
- `cat`: 分类,取值 `image` / `wallpaper` / `video` / `audio` / `game` / `texture` / `motion` / `character` / `ui` / `icon` / `font` / `tool` / `book` / `edu` / `skill` / `mcp` / `prompt` / `aitool` / `voice` / `noise` / `direct` / `pixel` / `ocean`
